...
function mcImgSlider(i) {
		...
		var I = function(a) {
			...
			Cb = function(a, c) {
				for (
					var e, f, g, b = a[d];
					b;
					e = parseInt(Math.random() * b), f = a[--b], a[b] = a[e], a[e] = f, g = c[b], c[b] = c[e], c[e] = g
				);
				return [a, c];
			},
			Bb = function(a, c, b) {
	...
	lb.prototype = {
		...
		q: function(b, e) {
			for (
				var f = 0,
					g = e < 7 ? { height: 0, opacity: -0.4 } : { width: 0, opacity: 0 },
					i = { height: s, opacity: 1 },
					a = 0,
					h = b[d];
				a < h;
				a++
				...
				for (var h = j, k = 0; k < a.m; k++) {
					if (h >= 0 && h < a.g) {
						...
						eb(
							...
							{ width: l.w, height: l.h, opacity: 1 },
							m
						);
						o++;
					}
		...
		u: function(a, i) {
			...
			for (var f = 0, b = 0, j = a[d]; b < j; b++) {
				...
				if (i == 16) {
					a[b][c][r] = a[b][c][q] = "0px";
					var g = { width: 0, height: 0, opacity: 0 },
						h = { width: e.w, height: e.h, opacity: 1 };
				} else {
					g = { opacity: 0 };
					h = { opacity: 1 };
				}
