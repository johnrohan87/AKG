...
!(function(i, h, m, e, d, k, f) {
	...
	var g = (i.$Jease$ = {
		...
		$InOutElastic: function(a) {
			...
			var b = 0.45,
				d = 0.1125;
			return (a *= 2) < 1
				? -0.5 * c.B(2, 10 * (a -= 1)) * c.gb(((a - d) * 2 * c.u) / b)
				: c.B(2, -10 * (a -= 1)) * c.gb(((a - d) * 2 * c.u) / b) * 0.5 + 1;
		},
		$InBack: function(a) {
			var b = 1.70158;
			return a * a * ((b + 1) * a - b);
		...
		function D(a) {
			try {
				return (
					...
					!a.nodeType &&
					a != a.window &&
					(!a.constructor || {}.hasOwnProperty.call(a.constructor.prototype, "isPrototypeOf"))
				);
			} catch (b) {}
		}
		function Db(a, b) {
	...
	var u = (i.$JssorSlideshowFormations$ = new function() {
		...
		i.$FormationCircle = function(d) {
			for (var e = d.$Cols || 1, f = d.$Rows || 1, g = [], a, i = e / 2 - 0.5, j = f / 2 - 0.5, b = 0; b < e; b++)
				...
				if (!a.Ic[f]) {
					...
					for (var d = 0; d < a.$Cols; d++)
						for (var e = 0; e < a.$Rows; e++)
							a.Ic[f][e + "," + d] = { y: e * b, t: d * c + c, m: e * b + b, x: d * c };
				}
				return a.Ic[f];
			};
			if (a.$Brother) {
				...
				b.f(c, function(a, d) {
					...
					if (c == "loading") R = a;
						...
						if (a.tagName != "STYLE" && a.tagName != "SCRIPT")
							nc[c || d] = new Rc(a, c == "slides", b.Fd(["slides", "thumbnavigator"])[c]);
					}
				});
				R && b.ib(R);
				R = R || b.bc(h);
